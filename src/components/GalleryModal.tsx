...
import { X, Upload, Plus, Trash2, Edit3, Image as ImageIcon, Link as LinkIcon, Folder, FolderPlus, Check } from 'lucide-react';
...
export default function GalleryModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      className="absolute inset-0 z-50 bg-[#050505] flex flex-col overflow-y-auto custom-scrollbar"
    >
      ...
      <div className="p-4 md:p-6 flex items-center justify-end shrink-0 relative z-10 border-b border-white/5">
        <div className="flex items-center gap-3">
          <select 
            ...
            className="w-48 bg-white/5 border border-white/10 rounded-lg p-2.5 text-sm text-white/80 outline-none focus:border-pink-500/50 hover:bg-white/10 transition-colors cursor-pointer appearance-none [&>option]:bg-[#0a0a0a] [&>option]:text-white"
          >
          ...
          <button 
            onClick={onClose} 
            className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-white/50 hover:text-white transition-all cursor-pointer backdrop-blur-md"
            title="Đóng (Phím Esc)"
          >
            <X size={20} />
          </button>
            ...
            {displayedImages.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {displayedImages.map((img: any) => (
                  <div 
                    key={img.id} 
                    className="group relative aspect-[2/3] rounded-xl overflow-hidden bg-black/40 border border-white/10 cursor-pointer flex-shrink-0 min-h-[150px]"
                    onClick={() => {
                  ...
                  >
                    ...
                    {!gallery.devImages?.some((d: any) => d.id === img.id) && (
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
